...
echo -e "#jobtask\tsample\tfhla" > jobtask_insample_inhla.core_generate_all_matches_coord.txt
...
for i in intables_batch_netmhcpan/$intablename"_"*; do
    ...
    sbatch --array=1-$subbatchnfiles%$subbatchnfiles $scriptdir/core_generate_all_matches_coord.sh \
               $i \
               $config \
               $scriptdir
done
