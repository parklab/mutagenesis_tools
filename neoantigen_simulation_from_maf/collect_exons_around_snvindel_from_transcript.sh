...
bedtools intersect -wb -a <(cut -f12,15-16 bkpt.$jobtask.gene.gtf) -b $exongtf > bkpt.$jobtask.exon.gtf
# 2. get unique genes
...
echo -e "jobtask\tgene\tfasta" > jobtask_gene_fasta.txt
# echo -e "#transcripts" > $jobtask.unique_transcripts.txt
while read gene; do
    ...
    # pull out transcripts
    # bedtools intersect -wb -a .temp.outside_lesion.bed -b $exongtf | cut -f18 | sed "s/\"; /\t/g" | sed "s/transcript_id \"//g" | cut -f2 | sort -k1,1 | uniq
    
    bedtools intersect -a $exongtf -b .temp.outside_lesion.bed > .temp.outside_lesion.exons.gtf
    cut -f9 .temp.outside_lesion.exons.gtf | sed "s/\"; /\t/g" | sed "s/transcript_id \"//g" | cut -f2 | sort -k1,1 | uniq > .temp.unique_transcripts.txt
    while read transcript; do
        echo $transcript
    done < .temp.unique_transcripts.txt
    ...
    bash $scriptdir/generate_lesion_sequence_from_exons.sh .temp.lesion_gene.bed $config $jobtask $gene
